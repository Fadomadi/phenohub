...
const GalleryPreview = ({ images, name, className }: GalleryPreviewProps) => {
    ...
    <Fragment>
      <div className={`grid grid-cols-3 gap-2 ${className ?? ""}`}>
        {sanitizedImages.slice(0, 6).map((image, index) => (
          ...
          >
            <span className="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-black/10 opacity-0 transition-opacity group-hover:opacity-100" />
            <Image
              src={image}
      ...
      {isOpen && sanitizedImages[activeIndex] && (
        <div
          className="fixed inset-0 z-[90] flex items-center justify-center bg-black/75 px-4 py-8"
          role="dialog"
          aria-modal="true"
        ...
        >
          <button
            type="button"
            onClick={closeModal}
            className="absolute right-6 top-6 flex h-11 w-11 items-center justify-center rounded-full bg-white/90 text-gray-600 shadow-lg transition hover:bg-white md:right-8 md:top-8"
          ...
          {sanitizedImages.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 top-1/2 hidden -translate-y-1/2 items-center justify-center rounded-full border border-white/40 bg-white/20 p-3 text-white transition hover:bg-white/30 md:flex"
              ...
              <button
                type="button"
                onClick={gotoNext}
                className="absolute right-4 top-1/2 hidden -translate-y-1/2 items-center justify-center rounded-full border border-white/40 bg-white/20 p-3 text-white transition hover:bg-white/30 md:flex"
