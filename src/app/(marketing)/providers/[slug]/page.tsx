...
const ProviderDetailPage = async ({ params }: ProviderPageProps) => {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <section className="mb-10 rounded-3xl border border-gray-200 bg-white/95 p-6 shadow-lg shadow-green-100/40 theme-dark:border-slate-800 theme-dark:bg-slate-900/85 theme-dark:shadow-slate-950/40 md:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
          <div className="flex-1 space-y-4">
            <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
              ...
              <span className="rounded-full bg-gray-100 px-3 py-1 text-gray-600 theme-dark:bg-slate-800 theme-dark:text-slate-300">
                {provider.countryFlag} {provider.country}
              </span>
              <span className="rounded-full bg-blue-100 px-3 py-1 text-blue-700 theme-dark:bg-slate-800 theme-dark:text-sky-200">
                {reportCount} Erfahrungsberichte
              </span>
            </div>

            <h1 className="text-3xl font-semibold text-gray-900 theme-dark:text-slate-100 md:text-4xl">
              {provider.name}
            </h1>

            <p className="text-base leading-relaxed text-gray-600 theme-dark:text-slate-300">
              {provider.bio?.trim() ||
                "Noch keine Beschreibung hinterlegt. Berichte ergänzen die Eindrücke der Community."}
            </p>
          </div>

  <div className="grid gap-3 rounded-3xl border border-gray-100 bg-gray-50 p-4 text-sm text-gray-700 theme-dark:border-slate-800 theme-dark:bg-slate-900/70 theme-dark:text-slate-200">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="rounded-2xl bg-white p-3 text-center shadow-sm theme-dark:bg-slate-900/80">
                <p className="text-[11px] uppercase text-gray-500">Versand</p>
                <p className="mt-1 text-base font-semibold text-gray-900 theme-dark:text-slate-100">
      ...
      <section className="mb-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div
            key={card.key}
            className="flex h-full flex-col justify-between rounded-3xl border border-gray-200 bg-white/95 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg theme-dark:border-slate-800 theme-dark:bg-slate-900/80"
          >
            <div className="flex items-center justify-between">
            ...
            <div className="mt-6 flex items-center justify-between">
              <div>
                <p className="text-lg font-semibold text-gray-900 theme-dark:text-slate-100">
                  {scoreDisplay(card.value)}
                </p>
                <p className="text-xs text-gray-500 theme-dark:text-slate-400">{card.helper}</p>
              </div>
      ...
      <section className="mb-10 rounded-3xl border border-gray-200 bg-white/95 p-6 shadow-sm theme-dark:border-slate-800 theme-dark:bg-slate-900/80">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 theme-dark:text-slate-400">
            {reportCount} Berichte insgesamt
          </span>
        </div>

        {recentReportCards.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-6 text-sm text-gray-500 theme-dark:border-slate-700 theme-dark:bg-slate-900/60 theme-dark:text-slate-300">
      ...
      <section className="rounded-3xl border border-gray-200 bg-white/95 p-6 shadow-sm theme-dark:border-slate-800 theme-dark:bg-slate-900/80">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Leaf className="h-4 w-4 text-green-600 theme-dark:text-sky-400" />
            <h2 className="text-lg font-semibold text-gray-900 theme-dark:text-slate-100">
                ...
                <div className="mb-3">
                  {offering.cultivarSlug ? (
                    <Link
                      ...
                      className="text-base font-semibold text-green-700 underline transition hover:text-green-800 theme-dark:text-sky-300 theme-dark:hover:text-sky-200"
                    >
                      {offering.cultivarName}
                    </Link>
                  ) : (
                    <p className="text-base font-semibold text-gray-900 theme-dark:text-slate-100">
                      {offering.cultivarName}
                    </p>
                  )}
                  {offering.breeder && (
                    <p className="text-xs uppercase tracking-wide text-gray-500 theme-dark:text-slate-400">
                      {offering.breeder}
                    </p>
                  )}
                </div>
                <div className="flex items-center justify-between text-sm text-gray-600 theme-dark:text-slate-300">
                  <span className="rounded-full bg-green-50 px-2 py-1 text-xs font-semibold text-green-700 theme-dark:bg-slate-800/70 theme-dark:text-sky-200">
                    {offering.category || "THC"}
                  </span>
                  <span className="text-base font-semibold text-gray-900 theme-dark:text-slate-100">
